...
**Design picker (2026-09-24, 0.16.0).** callenueve.com/design renders the real card face, back and box front with `callenueve-web/design/c9-render.js`, an IIFE bundle of `src/embed/render.tsx` (`npm run build:picker`, config `vite.picker.config.ts`). **Rebuild it and commit it whenever the card, back or box drawing changes**, or the site preview drifts from what the Studio prints. Choices become tokens through `utils/pickerDesign.ts` (`tokensFromDesign`), used by both the page and "Create project" on a lead (`createProjectFromLead`). The request lands in `inquiries.design` (migration 005).
